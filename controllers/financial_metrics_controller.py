...
class FinancialMetricsController:
    ...
    def get_financial_metrics(self, symbol: str):
        ...
        income_data = self.fetch_data(f"income-statement/{symbol}?limit=3")
        ratio_data = self.fetch_data(f"ratios/{symbol}?limit=3")
        
        # Get current and next 2 years forecast data
        forecast_data = self.fetch_data(f"forecast/income-statement/{symbol}?limit=3")
        analyst_estimates = self.fetch_data(f"analyst-estimates/{symbol}?period=annual&limit=3")
        ...
        for i in range(len(income_data)):
            ...
            result.append({
                ...
                "ebitdaMargin": self.format_percent(ratio_data[i].get("ebitdaMargin") if i < len(ratio_data) else None),
                "roe": self.format_percent(ratio_data[i].get("returnOnEquity") if i < len(ratio_data) else None)
            })
